    ...
    for epoch in range(args.train_epochs):
        ...
        for batch_idx, (imgs, img_scales, im_shapes, gt_path_indexs, im_infos) in enumerate(train_loader):
            ...
            if (batch_idx % args.show_step == 0):
                log = '({epoch}/{epochs}/{batch_i}/{all_batch}) | loss_tatal: {loss1:.4f} | loss_cls: {loss2:.4f} | loss_ver: {loss3:.4f} | loss_refine: {loss4:.4f} | Lr: {lr}'.format(
                    ...
                    loss2=loss_cls.item(), loss3=loss_ver.item(), loss4=loss_refine.item(), lr=scheduler.get_last_lr()[0])
                print(log)
                log_write.append([loss_tatal.item(),loss_cls.item(),loss_ver.item(),loss_refine.item(),scheduler.get_last_lr()[0]])
        ...
        print(
            ...
            format(loss1=np.mean(loss_total_list), loss2=np.mean(loss_cls_list), loss3=np.mean(loss_ver_list),
                   loss4=np.mean(loss_refine_list), lr=scheduler.get_last_lr()[0]))
        log_write.append([np.mean(loss_total_list),np.mean(loss_cls_list),np.mean(loss_ver_list),np.mean(loss_refine_list),scheduler.get_last_lr()[0]])
        print('-------------------------------------------------------------------------------------------------------')
        log_write.set_split(['---------','----------','--------','----------','--------'])
...
if __name__=="__main__":
    ...
    parser.add_argument('--size_list', nargs='?', type=int, default = [524], help='img max Size when train') #[768,928,1088,1200,1360]
    ...
    parser.add_argument('--train_epochs', nargs='?', type=int, default=100, help='how epoch to train')
